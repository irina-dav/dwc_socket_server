...
namespace DocsvisionSocketServer
{
    class DocsvisionDocumentFiles
    {
        ...
        public DocsvisionDocumentFiles(DocsvisionDocument dvDoc, string taskKind)
        {

            if (taskKind == "Ознакомление")
            {
                //RowData mainInfo_rd = parentDoc_cardData.Sections[CardDefs.CardDocument.MainInfo.ID].FirstRow;
                string refListId = dvDoc.GetMainInfoFieldString("ReferenceList");
            ...
            {
                files_rdc.AddRange(dvDoc.GetSectionData(CardDefs.CardDocument.Files.ID.ToString()).Rows.Cast<RowData>());
            }
    ...
    class FileRow
    {
        ...
        public FileRow(RowData row)
        {
            ...
            CardData versionedFileCard_cardData = DocsvisionSessionManager.Session.CardManager.GetCardData(versionedFileCard_id);
